...
//              img);
      ...
      if(bmp!=null){
        ...
        debugStrBuilder.append(command);
        putBytesImages(command, " ".charAt(0));
      }else{
  ...
  private String fixMenu(String key, String value, char space, int charsOnLine) {
    ...
    return rightPad(key, charsOnLine - value.length(), space) + value + "\n";
  }
  ...
  private String fixText(String text, char fill, TicketBuilder.TextAlignment alignment, int charsOnLine) {
    ...
    switch (alignment) {
      case RIGHT:
        return leftPad(text, charsOnLine, fill) + "\n";
      case CENTER:
        return StringUtils.center(text, charsOnLine, fill) + "\n";
      default:
        return rightPad(text, charsOnLine, fill) + "\n";
    }
  }
